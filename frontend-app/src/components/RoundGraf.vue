			...
			jQuery(function ($) {
				$(function () {
					$("#doughnutChart").drawDoughnutChart([
						{
							title: "タンパク質",
							value: store.state.allInfo["protein"],
							color: "#FC4349",
						},
						{
							title: "脂質",
							value: store.state.allInfo["fat"],
							color: "#F7E248",
						},
						{
							title: "炭水化物",
							value: store.state.allInfo["carbohydrate"],
							color: "#D7DADB",
						},
					]);
				});
	...
	(function ($, undefined) {
		$.fn.drawDoughnutChart = function (data, options) {
			...
			for (var i = 0, len = data.length; i < len; i++) {
				segmentTotalKcal = store.state.allInfo["kcal"];
			...
			function pathMouseEnter(e) {
				...
				$tip
					.text(data[order].title + ": " + data[order].value + "グラム")
					.fadeIn(200);
			...
			function drawPieSegments(animationDecimal) {
				...
				drawDoughnutText(animationDecimal, segmentTotalKcal);
